import React, { useState } from "react";
import Form from "./Form";
import { ref, uploadBytes, getDownloadURL } from "firebase/storage";
import { storage } from "../../firebase";
import { v4 } from "uuid";
import { toast } from "react-toastify";
import "./Rescue.css";
const Res = () => {
  const [imageUpload, setImageUpload] = useState(null);//usestate initially set to null
  const [imageUrls, setImageUrls] = useState([
    "data:image/png;base64,",
  ]);
  const [isuploaded, setisuploaded] = useState("false");//not uploaded initially and hence set to false
  // const imagesListRef = ref(storage, "images/");
  // const [refer , setref] = useState("")
  const uploadFile = () => {
    if (imageUpload == null) return;
    const imageRef = ref(storage, `images/${v4()}`); //if no img is uploaded then it displays the placeholder img.
    // setref(imageRef)
    uploadBytes(imageRef, imageUpload).then((snapshot) => {
      getDownloadURL(snapshot.ref).then((url) => {
        setImageUrls([url]);//if img is uploaded and then the url gets stored to setimg url's.
        toast.success("Image uploaded Succesfully", {//get's a pop up @ the top right corner //
          position: "top-right",
        });
        setisuploaded(true);//when img uploaded and then the initially set state false changes to true.
      });
    });
  };
  return (
    <div>
      <>
        <div className="page">
          <div className="container">
            <h1 className="heading">Dog Rescue Panel</h1>
            <div className="img-holder">
              {/* {imageUrls.map((url) => { */}
              <img src={imageUrls} className="img" alt="img" />
              {/* })} */}
              <input
                type="file"
                accept="image/*"
                name="image-upload"
                id="input"
                onChange={(event) => {
                  setImageUpload(event.target.files[0]);//uploaded img gets stored to the array.
                }}
                style={{
                  width: "200px",
                  padding: "3px",
                  height: "30px",
                  borderRadius: "5px",
                  marginTop: "2rem",
                  fontWight: "400",
                  letterSpacing: "1.15px",
                  backgroundColor: "#ffd301",
                  color: "black",
                  border: "none",
                  outline: "none",
                  cursor: "pointer",
                }}
              />
              <button
                onClick={uploadFile}
                style={{
                  width: "100%",
                  height: "40px",
                  borderRadius: "5px",
                  marginTop: "2rem",
                  fontWight: "400",
                  letterSpacing: "1.15px",
                  backgroundColor: "#ffd301",
                  color: "black",
                  border: "none",
                  outline: "none",
                  cursor: "pointer",
                }}
              >
                Upload Image
              </button>
            </div>
          </div>
        </div>
        {isuploaded === true ? <Form imageRef={imageUrls} /> : null}
        {/* //when isuploaded becomes true,then the form grts displayed to user */}
      </>
    </div>
  );
};

export default Res;
